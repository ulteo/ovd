...
###########################################
%package -n ulteo-ovd-slaveserver-gateway
###########################################

Summary: Ulteo Open Virtual Desktop - gateway role for slave server
Group: Applications/System
Requires: python, ulteo-ovd-slaveserver-gateway, openssl, python-openssl

%description -n ulteo-ovd-slaveserver-gateway
Gateway role for the Ulteo OVD slave server.

%prep -n ulteo-ovd-slaveserver-gateway
%setup -q

%build -n ulteo-ovd-slaveserver-gateway
%{__python} setup.py build

%install -n ulteo-ovd-slaveserver-gateway
...
%post -n ulteo-ovd-slaveserver-gateway
...
%postun -n ulteo-ovd-slaveserver-gateway
CONFDIR=%{_sysconfdir}/ulteo/ovd
if [ "$1" = "0" ]; then
...
%clean -n ulteo-ovd-slaveserver-gateway
%{__rm} -rf $RPM_BUILD_ROOT

%files -n ulteo-ovd-slaveserver-gateway
%defattr(-,root,root)
%{_libdir}/python*/site-packages/ovd/Role/Gateway/*.py*
%{_libdir}/python*/site-packages/ovd_slaveserver_gateway-*.egg-info

%changelog -n ulteo-ovd-slaveserver-gateway
