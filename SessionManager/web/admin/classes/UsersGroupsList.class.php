...
class UsersGroupsList {
	...
	function getForm($form_params_ = array('action' => 'search')) {
		...
		$str.= '</tr><tr><td></td>';
		$str.= '<td>'._('Search in: ');
		$str.= '<input type="checkbox" name="search_fields[]" value="name"';
		if (in_array('name', $this->search_fields))
			$str.= ' checked="checked"';
		$str.= '>'._('Name').' ';
		$str.= '<input type="checkbox" name="search_fields[]" value="description"';
		if (in_array('description', $this->search_fields))
			$str.= ' checked="checked"';
		$str.= '>'._('Description').' ';
		$str.= '</td></tr>';
