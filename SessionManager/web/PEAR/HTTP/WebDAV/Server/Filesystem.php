<?php
    ...
    class HTTP_WebDAV_Server_Filesystem extends HTTP_WebDAV_Server 
    {
        ...
        function fileinfo($path) 
        {
            ...
            $query = "SELECT ns, name, value FROM ".$this->db_prefix."properties WHERE path = '$path'";
        ...
        function COPY($options, $del=false) 
        {
            ...
            if ($del) {
                ...
                $query = "INSERT INTO ".$this->db_prefix."properties SELECT ... FROM properties WHERE path = '".$options['path']."'";
            }
        ...
        function PROPPATCH(&$options) 
        {
            ...
            foreach($options["props"] as $key => $prop) {
                if ($prop["ns"] == "DAV:") {
                    ...
                    if (isset($prop["val"])) {
                        $query = "REPLACE INTO ".$this->db_prefix."properties SET path = '$options[path]', name = '$prop[name]', ns= '$prop[ns]', value = '$prop[val]'";
                        error_log($query);
                    } else {
                        $query = "DELETE FROM ".$this->db_prefix."properties WHERE path = '$options[path]' AND name = '$prop[name]' AND ns = '$prop[ns]'";
                    }       
                    mysql_query($query);
                }
            }
        ...
        function LOCK(&$options) 
        {
            if (isset($options["update"])) { // Lock Update
                $query = "UPDATE ".$this->db_prefix."locks SET expires = ".(time()+300);
                mysql_query($query);
