...
class Application_webapp extends Application{
	protected $default_browser;

	public function __construct($id_, $name_, $description_) {
		Logger::debug('main', "Application_webapp::construct('$id_','$name_','$description_')");
		parent::__construct($id_, $name_, $description_, 'webapp', $name_, NULL, true, $name_.'.webapp');
		...
		if (!is_array($default_browser)) {
			Logger::error('main', 'Application_webapp::construct failed to get default_browser preferences');
		}
	...
	public function toXML($ApS=NULL) {
		...
		if ( $this->hasAttribute('executable_path'))
			$executable_node->setAttribute('command', 'firefox '.$this->attributes['executable_path']); // executable_path <=> slaveserver_host
