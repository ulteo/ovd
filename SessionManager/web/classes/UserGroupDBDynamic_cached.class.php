...
class UserGroupDBDynamic_cached extends UserGroupDBDynamic {
	...
	public function __construct() {
		...
		if ($prefs) {
			...
			if (is_array($mysql_conf)) {
				$this->table =  $mysql_conf['prefix'].'usergroup_dynamic_cached';
			}
			else
				$this->table = NULL;
		}
	...
	public function updateCache($usergroup_) {
		if ( mktime() > ( $usergroup_->last_update + $usergroup_->schedule )) {
			Logger::error('main', 'UserGroupDBDynamic_cached::updateCache usergroup (id='.$usergroup_->getUniqueID().') must update his cache');
	...
	public static function init($prefs_) {
		...
		if (!is_array($mysql_conf)) {
			Logger::error('main', 'UserGroupDBDynamic_cached::init mysql conf not valid');
			return false;
		}
