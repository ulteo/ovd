...
final public class ConvertToString {
	...
	final static private String convertMapToString(Map currentField,int indent,PdfObjectReader currentPdfFile) {
		...
		while(iter.hasNext()){
			...
			if(next instanceof Map){
                ret.append(key).append('=').append(convertMapToString((Map) next, indent + 1, currentPdfFile));
			}else{
			    if(currentPdfFile!=null){
			    	if(key.equals("Parent")){
			    		...
			    		if(obj instanceof Map){
	                        ret.append(key).append('=').append(convertMapToString((Map) obj, indent + 1, currentPdfFile));
			    		}else {
    ...
    public static void printStackTrace(int level) {
    	printStackTrace(2,level+1,false);
    }
    ...
    public static void printStackTrace(int startLevel,int endLevel,boolean err) {
		...
		for(int i=startLevel;i<=endLevel;i++){//start at 1 to ignore ConvertToString.printStackTrace line
			if(err)
