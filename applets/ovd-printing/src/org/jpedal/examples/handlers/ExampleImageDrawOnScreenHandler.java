...
public class ExampleImageDrawOnScreenHandler implements org.jpedal.external.ImageHandler {
    ...
    public boolean drawImageOnscreen(BufferedImage image, int optionsApplied, AffineTransform upside_down,
                                     String currentImageFile, Graphics2D g2, boolean renderDirect,
                                     ObjectStore objectStoreRef, boolean isPrinting) {
...
//						System.out.println(values[4]*image.getHeight()/image.getWidth());
                    ...
                    if(JAIHelper.isJAIused()){
                        ...
                        if(!imageProcessed && currentImageFile!=null){  //try it via tiled image if failed (thanks to Cesssna for starting code)
							...
							try{
                                ...
                                //@jason - I've added a value here
                                float quality=1f;
                                /***/
								RenderedImage ri;

								com.sun.media.jai.codec.SeekableStream s = new com.sun.media.jai.codec.FileSeekableStream(new File(objectStoreRef.getFileForCachedImage(currentImageFile)));
					...
					if(imageProcessed){
                        ...
                        if(isPrinting && dy==0){ //adjust to fit
                            ...
                            //System.out.println(dx+" "+dy+" "+g2.getTransform());
...
public static synchronized BufferedImage getAffineTransform(RenderedImage
ri, AffineTransform src2me, RenderingHints hints, float quality) {
...
((wr.getHeight()-(kh*2)) > 0) )
                        $ri = CropDescriptor.create(
                                ...
                                new Float(kw),
                                new Float(kh),
                                new Float(wr.getWidth()-(kw*2)),
                                new Float((float)wr.getHeight()-(kh*2)),
                                hints
                        );
                }
