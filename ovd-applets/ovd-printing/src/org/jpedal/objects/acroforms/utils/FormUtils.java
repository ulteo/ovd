...
public class FormUtils {
	 ...
	 * @param allFields 
	 */
	public static int[] sortGroupLargestFirst(int[] array, float[][] boundingBoxs) {
		
		return sortCompsDesending(array,boundingBoxs);
	}
     ...
     */
    private static int[] sortCompsDesending(int[] array, float[][] boundingBoxs) {
        //reference
        //Sorts.quicksort(new int[1],new int[1]);
        ...
        while (left > 0) {
            ...
            //go through tree starting with leaves and going up
            siftCompsDesending(array,boundingBoxs, left, right);
        }
     ...
     * This Is Called from That Method ONLY
     */
    private static void siftCompsDesending(int[] array, float[][] boundingBoxs, int left, int right) {
        int currentLeft;
        int primaryTMP;
        int childL;
        ...
        //Find a[left]'s larger child
        if ((childL < right) && shouldSwapControlDesending(array[childL], array[childL + 1],boundingBoxs)) {
            childL = childL + 1;
        }
        //assert: a[childL] is larger child

        //sift temp to be in correct place in highest on leftMost and arranged as tree
        while ((childL <= right) && shouldSwapControlDesending(primaryTMP, array[childL],boundingBoxs)) {
            //assign highest item to leftmost position
     ...
     */
    private static boolean shouldSwapControlDesending(int arg1, int arg2,float[][] boundingBoxs) {

        /**
         * sorts by area, same as acrobat
         return (first.width*first.height)>(second.width*second.height);
         */
        return (boundingBoxs[arg1][2] * boundingBoxs[arg1][3]) > (boundingBoxs[arg2][2] * boundingBoxs[arg2][3]);
    }
