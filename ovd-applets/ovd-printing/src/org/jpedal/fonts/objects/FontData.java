...
public class FontData {
    ...
    public FontData(String cachedFile){//, byte[] stream) {
        ...
        /**
        //test
        byte[] allData=new byte[fullLength];
        try {
            fontFile.read(allData);
        } catch (IOException e) {
        ...
        try{
            ...
            int arrayLength=allData.length;

            if(arrayLength!=stream.length){
                System.out.println("Wrong lengths"+arrayLength+" "+stream.length);
                System.exit(1);
            }

            boolean failed=false;
            for(int ii=0;ii<arrayLength;ii++){
                if((stream[ii]!=allData[ii])){
                    //System.out.println(startStreamPointer+"<>"+endStreamPointer);
                    System.out.println("X1 Not same value at "+ii+" =="+stream[ii]+" "+allData[ii]);
                    failed=true;
                    System.exit(1);
                }
            }
