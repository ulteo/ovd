...
import org.jpedal.objects.raw.PdfStrings;
...
public class PdfFont implements Serializable {
	...
	final private void handleCIDEncoding(PdfObject Encoding) throws PdfFontException
	{
        ...
        String encodingName=PdfCIDEncodings.getNameForEncoding(encodingType);
        ...
        String CMapName=Encoding.getStringValue(PdfDictionary.CMapName);
        if(CMapName!=null){

        	byte[] stream=currentPdfFile.readStream(Encoding,true,true,false, false,false);        	
		...
		if(isIdentity){
			...
			//get settings
			isDoubleByte=PdfCIDEncodings.isDoubleByte(encodingType);
            ...
            if (char_mapping != null)
				...
				if (char_mapping != null)
                    ...
                    if(mapped_char.length()==1){
                        ...
                        //handle some odd mappings in Type3
                        if(return_value.length()==0 && fontTypes==StandardFonts.TYPE3)// && !StandardFonts.isValidGlyphName(char_mapping))
...
//				System.exit(1);
	...
	public void createCIDFont(PdfObject pdfObject, PdfObject Descendent) throws PdfFontException{
		...
		if(ToUnicode!=null)
            readUnicode(currentPdfFile.readStream(ToUnicode,true,true,false, false,false),fontID);
        
		/**read widths*/
		//@speed may need optimising - done as string for moment
		String widths=Descendent.getStringValue(PdfDictionary.W);
		if(widths!=null)
		...
		if(CIDToGID!=null){	
			byte[] stream=currentPdfFile.readStream(CIDToGID,true,true,false, false,false);
			if(stream!=null){
				...
				handleCIDEncoding(new PdfFontObject(PdfDictionary.Identity_H));		
		}
...
defaultFont="Hiragino Mincho Pro";
	...
	public void createFont(PdfObject pdfObject, String fontID, boolean renderPage, ObjectStore objectStore, Map substitutedFonts) throws Exception{
        ...
        setEncoding(pdfObject, fontID, pdfFontDescriptor);
	...
	protected void setName(PdfObject pdfObject, String fontID) {
		 ...
		 */
		// Get fontName
		String baseFontName= pdfObject.getStringValue(PdfDictionary.BaseFont);
		if(baseFontName==null)
			baseFontName= pdfObject.getStringValue(PdfDictionary.FontName);
		if (baseFontName == null)
		 ...
		 **/
		String truncatedName= pdfObject.getStringValue(PdfDictionary.BaseFont,PdfStrings.REMOVEPOSTSCRIPTPREFIX);
		if(truncatedName==null)
			truncatedName= pdfObject.getStringValue(PdfDictionary.FontName,PdfStrings.REMOVEPOSTSCRIPTPREFIX);
		if (truncatedName == null)
	...
	protected void setEncoding(PdfObject pdfObject, String fontID, PdfObject pdfFontDescriptor) {
	...
	protected final void handleFontEncoding(PdfObject pdfObject,PdfObject Encoding){
		 ...
		 */
		PdfArrayIterator Diffs=Encoding.getArrayIterator(PdfDictionary.Differences);
		if (Diffs != null && Diffs.getTokenCount()>0) {
			...
			while (Diffs.hasMoreTokens()) {
				...
				if(type==PdfArrayIterator.TYPE_INTEGER)
					...
					putMappedChar( pointer,Diffs.getNextValueAsFontChar(pointer));
                    pointer++;
				}
			}
		...
		try {
            ...
            while (true) {
                ...
                if (ptr>=length)
                    ...
                    while(data[ptr]==62 || data[ptr]==32 || data[ptr]==10 || data[ptr]==13)
                        ptr++;
                    ...
                    if(inDefinition==1) {
                        ...
                        if(entryCount==2){
                            unicodeMappings[value[0]]= new String(String.valueOf((char)value[inDefinition]));
                        }else{
                        ...
                        for (int i = value[0]; i < value[1] + 1; i++)
                            unicodeMappings[i]= new String(String.valueOf((char)(value[inDefinition]+i-value[0])));
                    }
                }
    ...
    public Rectangle getBoundingBox() {
        ...
        //<start-os>
        //System.out.println("getBoundingBox() base method called for "+this);
        //<end-os>
