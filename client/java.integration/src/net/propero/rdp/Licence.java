...
public class Licence {
	...
	Licence(Secure s, Options opt_, Common common_){
	secure = s;
	this.opt = opt_;
	this.common = common_;
	...
	*/
	...
	public byte[] generate_hwid() throws UnsupportedEncodingException{ 
	   ...
	   secure.setLittleEndian32(hwid, 2);
		   byte[] name = this.opt.hostname.getBytes("US-ASCII");
   ...
   public void process(RdpPacket_Localised data) throws RdesktopException, IOException, CryptoException {
	   ...
	   byte[] host = this.opt.hostname.getBytes("US-ASCII");
	   byte[] user = this.opt.username.getBytes("US-ASCII");
	   ...
	   if(!this.opt.built_in_licence && this.opt.load_licence){
    ...
    /**
     * Handle a licence issued by the server, save to disk if this.opt.save_licence
	...
	public void process_issue(RdpPacket_Localised data)throws CryptoException {
		...
		if(this.opt.save_licence) save_licence(data,length-2);
		}
	...
	public void send_request(byte[] client_random, byte[] rsa_data, byte[] username, byte[] hostname) throws RdesktopException, IOException, CryptoException {
		...
		if(this.opt.built_in_licence && (!this.opt.load_licence) && (!this.opt.save_licence)){
	...
	byte[] load_licence(){
		...
		return (new LicenceStore_Localised(this.opt, this.common)).load_licence();
	}
...
void save_licence(RdpPacket_Localised data, int length){
    ...
    new LicenceStore_Localised(this.opt, this.common).save_licence(databytes);
