...
public final class DynamicVectorRenderer {
    ...
    /**track items painted to reduce unneccasry calls*/
...
//	}
	...
	public static boolean marksNewCode=false;
	...
	final public void renderEmbeddedText(GraphicsState gs,int text_fill_type,Object rawglyph,int glyphType,
		...
		if((glyph!=null)){
			...
			if ((text_fill_type & GraphicsState.STROKE) == GraphicsState.STROKE){
				...
				try{
					glyph.render(GraphicsState.STROKE,g2, scaling);
				}catch(Exception e){
	...
	public final void renderImage(AffineTransform imageAf, BufferedImage image,float alpha,
		...
		if(renderDirect){
			...
			areas.addElement(new Rectangle((int)currentGraphicsState.CTM[2][0],
					(int)currentGraphicsState.CTM[2][1],iw,ih));
		}
	...
	public Rectangle paint(Graphics2D g2,Rectangle[] highlights,AffineTransform viewScaling,
		...
		for(int i=0;i<count;i++){
			...
			if(type>0){
				...
				if(type<7 && (userAnnot!=null)&&(currentArea!=null)&&
						((!userAnnot.intersects(currentArea)))){
					ignoreItem=true;
				...
				if(ignoreItem || (lastItemPainted!=-1 && i<lastItemPainted)){
					...
					case TEXT:
						...
						if(!drawJustHighlights || textHighlightsX!=null)
							renderText(x,y, currentTR,(Area)currentObject,g2,
									setHighlightForGlyph(currentArea,objectTypes,highlights, i),
									textStrokeCol,textFillCol,strokeOpacity,fillOpacity);
						break;
					...
					case IMAGE:
						...
						if(!isType3Font && objectStoreRef.isRawImageDataSaved(key) && useHiResImageForDisplay){
							...
							defaultX=((Integer)getObjectStore().getRawImageDataParameter(key,ObjectStore.IMAGE_pX)).intValue();
							pX=(int)(defaultX*scalingToUse);

							defaultY=((Integer)getObjectStore().getRawImageDataParameter(key,ObjectStore.IMAGE_pY)).intValue();
							pY=(int)(defaultY*scalingToUse);

							w1=((Integer)getObjectStore().getRawImageDataParameter(key,ObjectStore.IMAGE_WIDTH)).intValue();
							h1=((Integer)getObjectStore().getRawImageDataParameter(key,ObjectStore.IMAGE_HEIGHT)).intValue();
							...
							if(pX>0){
								...
								if((scaling>1f || lastScaling>1f)&& sampling>=1 && (lastScaling!=scaling)){
									...
									{//if(d==1){
										...
										byte[] data=getObjectStore().getRawImageData(key);
				...
				currentObject=image;
						...
						if(!drawJustHighlights){
							...
							if(this.useHiResImageForDisplay){
								...
								if(samplingDebug)
									System.out.println("Hires scaling="+scaling+" sampling="+sampling+" default="+defaultSampling);

								AffineTransform imageAf=null;
								if(sampling>=1 && scaling>1 && w1>0){

									double aa=((float)sampling)/defaultSampling;

									if(samplingDebug)
										System.out.println(defaultSampling+" "+aa+" <<afValues>> "+afValues1[afCount]+" "+afValues2[afCount]+" "+afValues3[afCount]+" "+afValues4[afCount]);

									imageAf=aff=new AffineTransform(afValues1[afCount]*aa,afValues2[afCount]*aa,
											afValues3[afCount]*aa,afValues4[afCount]*aa,x,y);
								}else
									imageAf=aff=new AffineTransform(afValues1[afCount],afValues2[afCount],afValues3[afCount],afValues4[afCount],x,y);

								if(samplingDebug)
									System.out.println("imageAf="+imageAf);
								...
								if(currentObject==null){
									try{
										...
										if(currentObject==null){
											...
											if(rawKey==null)
												currentObject=objectStoreRef.loadStoredImage(pageNumber+"_HIRES_"+i);
											else
												currentObject=objectStoreRef.loadStoredImage(pageNumber+"_HIRES_"+i+ '_' +rawKey);
											...
											//recache
											if(!isPrinting  && displayMode== Display.SINGLE_PAGE){
										...
										//<start-jfr>
										//fix for bug in Mac OS
										if((PdfDecoder.isRunningOnMac)&&(img!=null)&&(img.getType()==0))
											img=ColorSpaceConvertor.convertToARGB(img);
										//<end-jfr>
								...
								if(img!=null){
									if(samplingDebug)
										System.out.println("hires sampling="+sampling+" option a "+imageAf+" "+" "+img);

									renderImage(imageAf,img,fillOpacity,null,g2,x,y,currentImageOption);
...
//				(highlights[i2].intersects(area))) {
	...
	final public void drawText(float[][] Trm,int fontSize,Object transformedGlyph2,GraphicsState currentGraphicsState) {
		...
		if(transformedGlyph2 instanceof Area) {
			...
			checkWidth(((Area)transformedGlyph2).getBounds());
	...
	final public void drawImage(int pageNumber,BufferedImage image,
		...
		if(useHiResImageForDisplay){
			...
			if(!alreadyCached || cachedWidths.get(key)==null){
				...
				w=((Integer)cachedWidths.get(key)).intValue();
				h=((Integer)cachedHeights.get(key)).intValue();
			}

			if(CTM[0][0]==0 && (CTM[1][1]==0)){

				boolean isRotated=isRotated(CTM);

				if(isRotated && ((optionsApplied & PDFImageProcessing.IMAGE_ROTATED) !=PDFImageProcessing.IMAGE_ROTATED)){    //fix for odd rotated behaviour
					...
					//if(1==1 || !PdfDecoder.isRunningOnMac){
					float[][] scaleDown={{0,1f/(float)h,0},{1f/(float)w,0,0},{0,0,1}};
					CTM=Matrix.multiply(scaleDown,CTM);
//					}else{
//					float[][] scaleDown={{0,1f/(float)w,0},{1f/(float)h,0,0},{0,0,1}};
//					CTM=Matrix.multiply(scaleDown,CTM);
//					}
				}else{
					float[][] scaleDown={{0,1f/(float)w,0},{1f/(float)h,0,0},{0,0,1}};
				...
				float[][] scaleDown={{1f/(float)w,0,0},{0,1f/(float)h,0},{0,0,1}};
				CTM=Matrix.multiply(scaleDown,CTM);
			}
			...
			if(PdfDecoder.clipOnMac && PdfDecoder.isRunningOnMac){
				...
				if(dy==0)
					dy=1;
				upside_down=new AffineTransform(dx,0,0,dy,0,0);
				if((CTM[0][1]>0)&&(CTM[1][0]<0))
					x=x-w;
			...
			if(!alreadyCached){
				...
				if(rawKey==null){
					...
					imageIDtoName.put(new Integer(currentItem),pageNumber+"_HIRES_"+currentItem);
				}else{
					objectStoreRef.saveStoredImage(pageNumber+"_HIRES_"+currentItem+ '_' +rawKey,image,false,false,"tif");
					imageIDtoName.put(new Integer(currentItem),pageNumber+"_HIRES_"+currentItem+ '_' +rawKey);
				}
				...
				cachedWidths.put(key,new Integer(w));
				cachedHeights.put(key,new Integer(h));
			}
	...
	public Rectangle getOccupiedArea(){
		return new Rectangle(pageX1,pageY1,(pageX2-pageY1),(pageY1-pageY2));
	}
	...
	public void drawEmbeddedText(float[][] Trm,int fontSize,PdfGlyph glyph,int type,
		...
		//if(nextAf[1]<0 && nextAf[2]<0)
		//area=new Rectangle((int)Trm[2][0],(int)Trm[2][1],-fontSize,-fontSize);
	...
	public void drawFontBounds(Rectangle newfontBB) {
		...
		//if(newfontBB.getBounds().height>2000)
		//System.exit(1);
