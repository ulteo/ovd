...
public class Type1 extends PdfFont {
		...
		while ((line = br.readLine()) != null) {
			...
			if (line.startsWith("dup")) {
				...
				if (info.countTokens() >= 3) {
					...
					code = Integer.parseInt(info.nextToken()); //code
	...
	private void readEncodedContent(byte[] cont) throws Exception {
		...
		for (i = 4; i < size; i++) { //find the start of /CharStrings
        ...
        if(start==-1){
            ...
            extractFontData(skipBytes,cont,start,rd,l,nd);
...
//        }
        while(start<=end){
            ...
            /**
			if(glyph.toString().equals("F")){
				//for(int ii=-5;ii<byteCount;ii++)
				//	System.out.println(ii+" "+cont[start+ii]+" "+(char)cont[start+ii]);
				System.out.println("Skip="+skipBytes);
				for(int jj=0;jj<10;jj++)
				    System.out.println(jj+" "+stream[jj]);
			//System.exit(1);
			}/**/
	...
	final private byte[] getStream(int skipBytes,int start,int byteCount,byte[] cont) throws IOException{
