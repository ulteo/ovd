...
public class PdfJavaGlyphs implements PdfGlyphs,Serializable{
	 ...
	 * @param currentWidth
	 */
	public Area getStandardGlyph(float[][]Trm, int rawInt, String displayValue, 
			float currentWidth, boolean flag,double[] values) {
		...
		if (transformedGlyph2 == null) {
			...
			if (flag) {
				...
				if(gv1!=null){
					...
					/**<end-13><start-14>
					 double glyphWidth=gv1.getLogicalBounds().getWidth()+(glyphX*2);
					 //<end-14>*/
			...
			if(transformedGlyph2!=null){
				...
				if(values!=null){
					...
					at =new AffineTransform(dX*Trm[0][0],dX*Trm[0][1],dY*Trm[1][0],dY*Trm[1][1] ,x,y);
					transformedGlyph2.transform( at);
				}
			}
...
//		}
	...
	public void setT3Glyph(int key, PdfGlyph glyph) {
