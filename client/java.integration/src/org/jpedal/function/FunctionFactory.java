...
public class FunctionFactory {
	...
	public static PDFFunction getFunction(PdfObject functionObj, PdfObjectReader currentPdfFile) {
		 ...
		 **/
        byte[] stream=functionObj.DecodedStream;
		...
		if(keys!=null){
			...
			for(int i=0;i<functionCount;i++){
                ...
                if(id.startsWith("<<")){
                    function=new PdfFunctionObject(1);
                    currentPdfFile.readDictionaryAsObject(function,"",0,keys[i], keys[i].length,"", true);           
                }else{
                    function=new PdfFunctionObject(id);
				    currentPdfFile.readObject(function, id,false,null);
                }
