 ...
 */
public class XFAFormStream extends AnnotStream implements FormDecoder {
    ...
    public XFAFormStream(Document configData, Document datasetData, Document templateData, PdfObjectReader inCurrentPdfFile) {
        config = configData;
        dataset = datasetData;
        template = templateData;

        type = DefaultAcroRenderer.XFAFORM;
        currentPdfFile = inCurrentPdfFile;
        init();
    }

    public FormObject[] createAppearanceString(FormObject[] forms, PdfObjectReader inCurrentPdfFile) {
//		System.out.println("xfa form="+ConvertToString.convertDocumentToString(template));
...
//System.out.println("xfa="+xfaForm.getFieldName()+" formname="+formName);
                    if (xfaForm.getFieldName().equals(formName)) {
...
//		System.out.println("config="+ConvertToString.convertDocumentToString(config));
        if (debugUnimplemented)
            System.out.println("config= NOT USED");
//		NodeList nodes = config.getChildNodes();
//		for(int i=0;i<nodes.getLength();i++){
//			parseNode(nodes.item(i));
//		}

//		System.out.println("dataset="+ConvertToString.convertDocumentToString(dataset));
        if (debugUnimplemented)
            System.out.println("dataset= NOT USED");
//		nodes = dataset.getChildNodes();
//		for(int i=0;i<nodes.getLength();i++){
//			parseNode(nodes.item(i));
//		}

//		NodeList nodes = template.getChildNodes();
//		System.out.println("template="+ConvertToString.convertDocumentToString(template));
//		System.out.println("-----------------------------------------------------------" +
//				"------------------------------------------------------------------------");
//		for(int i=0;i<nodes.getLength();i++){
//			parseNodeOld(nodes.item(i));
//		}
        
        ArrayList datalist = new ArrayList();
        parseNode(dataset,datalist);
//        Node[] nodes = new Node[datalist.size()];
//        datalist.toArray(nodes);
//
...
//		}
    ...
    private void parseNode(Node nodeToParse, ArrayList nodeList) {
     ...
     */
    private void setupNode(Iterator nodeIterator) {
        while (nodeIterator.hasNext()) {
            Node node = (Node) nodeIterator.next();
            String nodeName = node.getNodeName();
...
//				template(node,nodeIterator);
    ...
    private int resolveMeasurementToPoints(String attValue) {
        int val = 0;
...
//				System.out.println("y="+contentY+" h="+contentH);
    ...
    private Node findNodeIn(String searchName, Node nodeToParse) {
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            return findNodeIn(searchName, node);
        }
        return null;
    }
...
//    			System.out.println("   items="+tmpNode);
                ...
                formObject.setlistOfItems(listOfItems);
...
//    				formObject.setTextFont();
                ...
                if (tmp != null) {
                    ...
                    formObject.setBackgroundColor(generateColorFromString(tmp.getNodeValue()));
                }
