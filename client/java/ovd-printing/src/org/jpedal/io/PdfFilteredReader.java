...
public class PdfFilteredReader extends PdfFileReader {
    	...
    	if (filterCount>0){
    		...
    		while (filters.hasMoreTokens()) {
    			...
    			if (filterType==FlateDecode || filterType==Fl){
    				...
    				/** if it fails or not called, fall back to old version */
    				if (newData == null)
    					if(newCCITT  && 0==DecodeParms.getInt(PdfDictionary.K)){ 
    			...
    			if (debugCaching && cacheName != null && !isIgnored) {
    				...
    				if (debug)
    					System.out.println("data="+ count + " cacheName=" + cacheName);
    				...
    				try {
    					...
    					if (cachedData.length != count) {
    						System.out.println("Different sizes inMemory="+ count + " cached="+ cachedData.length);
    						System.exit(1);
    					}
	...
	private void setupCachedObjectForDecoding(byte[] data, String cacheName,
		...
		if ((data != null) && (debugCaching)) {
			...
			if (debug)
				System.out.println("Cached stream data length=" + data.length);

			if (tempFile2.length() != data.length) {

				if (debug) {
					System.out.println("inputs different lengths "
							+ tempFile2.length() + ' ' + data.length);
	 ...
	 * they are identical
	 */
	private byte[] runLengthDecode(byte[] data,BufferedInputStream bis,
	...
	*
     ...
     * @param cacheName
	 */
	final private byte[] JBIGDecode(BufferedInputStream bis,
     ...
     */
	private byte[] ccittDecode(byte[] data, Map values, int width,
	 ...
	 * ccitt decode using Sun class
     */
	private byte[] ccittDecode(byte[] data, PdfObject DecodeParms, int width,
			int height) throws Exception {
	...
	private byte[] sunCCITTDecode(byte[] data, boolean isBlack, int columns,
	 ...
	 */
	final private byte[] ascii85DecodeNEW(byte[] valuesRead) {
	 ...
	 * ascii85decode using our own implementation
	 */
	final private void ascii85Decode(BufferedInputStream bis,
				...
				if (nextValue == 122) {
					...
					if (!ignoreLastItem) {
						...
						for (int i3 = 0; i3 < 4; i3++) {
							...
							/**
							 * if(b!=test[cc]){ System.out.println(cc+" "+b+"
							 * "+test[cc]); System.exit(1); }
							 */
							streamCache.write(b);
						}
					}
	 ...
	 * asciihexdecode using our own implementation
	 */
	final private void asciiHexDecode(BufferedInputStream bis,
		...
		if (bis != null) {
			...
			try {
				...
				while (true) {
					...
					if (returnData != null) {
						// System.out.println(i+" = "+b+" "+(returnData[i] &
						// 255));
	 ...
	 */
	private void applyPredictorFunction(int mainPred, BufferedInputStream bis, 
		...
		try{
			...
			while(true){
				...
				//exit after all used
				if(bytesAvaiable<=byteCount)
					break;

				//set predictor
				predictor=mainPred;

				if(predictor==15){ //per line basis (otherwise we ignore first byte
				...
				}else if(predictor>=10){ //ignore but get rid of byte
					bis.read();
				}
