...
public class Bitmap {
    ...
    public int convertTo24(int colour){
        if(this.opt.server_bpp == 15) return convert15to24(colour);
        if(this.opt.server_bpp == 16) return convert16to24(colour);
        return colour;
    }
     ...
     * @return
     */
	public int cvalx(byte[] data, int offset, int Bpp) {
		int rv = 0;
        ...
        }else if(this.opt.server_bpp == 16){
            int lower = data[offset] & 0xFF;
            int full = (data[offset + 1] & 0xFF) << 8 | lower;          
     ...
     */
	public int[] convertImage(byte[] bitmap, int Bpp) {
		...
		for (int i = 0; i < out.length; i++) {
			if (Bpp == 1)
				...
				out[i] = ((bitmap[i * Bpp + 2] & 0xFF) << 16)
						...
						| (bitmap[i * Bpp] & 0xFF);
			out[i] = this.convertTo24(out[i]);
		}
		return out;
	}

	public Bitmap(Options opt_) {
		this.opt = opt_;
	}
    /**
	 ...
	 */
    public WrappedImage decompressImgDirect(int width, int height, int size, RdpPacket_Localised data, int Bpp, IndexColorModel cm, int left, int top, WrappedImage w) throws RdesktopException {
     ...
     * @throws RdesktopException
     */
    public Image decompressImg(int width, int height, int size, RdpPacket_Localised data, int Bpp, IndexColorModel cm) throws RdesktopException {
