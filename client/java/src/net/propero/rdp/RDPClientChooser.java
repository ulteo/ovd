...
public class RDPClientChooser {
     ...
     */
	public RDPClientChooser(Options opt_) {
		this.opt = opt_;
	...
	throws IOException 
	{
		...
		this.opt.windowTitle = "Remote Desktop Connection";
		...
		while ((c = g.getopt()) != -1) 
		{
			switch (c) {
				...
				case 'd':
					this.opt.domain = g.getOptarg();		 
					break;	
					
				case 'n':
					this.opt.hostname = g.getOptarg();
					break;

				case 'p':
					this.opt.password = g.getOptarg();
					break;	

				case 't':
					arg = g.getOptarg();
					try {
						this.opt.port = Integer.parseInt(arg);
					}
					catch (Exception e) {
					}
					break;
				...
				case 'T':
					this.opt.windowTitle = g.getOptarg().replace('_',' ');
					break;

				case 'u':
					this.opt.username = g.getOptarg();		 
					break;
		...
		if (g.getOptind() < args.length) {
			...
			if (colonat == -1){
				...
				this.opt.port = Integer.parseInt(args[args.length-1].substring(colonat+1));
			}
		}
		else {
		...
		String rdproot = "/var/tmp/RDP-"+this.opt.hostname+"-"+this.opt.port;
		...
		rdpConfigFile.write("full address:s:"+server+":"+this.opt.port+"\n");
		...
		rdpConfigFile.write("username:s:"+this.opt.username+"\n");
		rdpConfigFile.write("clear password:s:"+this.opt.password+"\n");
		rdpConfigFile.write("domain:s:"+this.opt.domain+"\n");
		...
		String[] mvcmd = {"/bin/sh", "-c", "mv "+rdproot+"/Remote\\ Desktop\\ Connection '"+rdproot+"/"+this.opt.windowTitle+"' >/dev/null 2>/dev/null"};
		...
		String[] rdpcmd = {"/bin/sh", "-c", "open -a '"+rdproot+"/"+this.opt.windowTitle+"' "+rdproot+"/Default.rdp >/dev/null 2>/dev/null"};
