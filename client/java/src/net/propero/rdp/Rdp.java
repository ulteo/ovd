...
public class Rdp {
     ...
     * @param data Packet containing capability set data at current read position
     */
    protected void processBitmapCaps(RdpPacket_Localised data) {
         ...
         */
        if (this.opt.server_bpp != bpp) {
            logger.warn("colour depth changed from " + this.opt.server_bpp
                    + " to " + bpp);
     ...
     */
    public Rdp(VChannels channels, Options opt_, Common common_) {
        this.opt = opt_;
        this.common = common_;
     ...
     */
    public void connect(String username, String server, int flags,
    ...
    private void sendLogonInfo(int flags, String domain, String username,
        ...
        if (!this.opt.use_rdp5 || 1 == this.opt.server_rdp_version) {
            ...
            data.setLittleEndian32(this.opt.rdp5_performanceflags); // out_uint32_le(s,
                                                                    // 0x0f);
            data.setLittleEndian32(0); // out_uint32(s, 0);
        }
    ...
    private void sendConfirmActive() throws RdesktopException, IOException,
        ...
        data.setLittleEndian16(this.common.mcs.getUserID() /* McsUserID() */+ 1001);
	...
	// Glyph capabilities
        ...
        this.common.secure.send(data, sec_flags);
    }
    ...
    private void sendBitmapCaps(RdpPacket_Localised data) {
        ...
        data.setLittleEndian16(this.opt.server_bpp); /* Preferred BPP */
        data.setLittleEndian16(1); /* Receive 1 BPP */
        data.setLittleEndian16(1); /* Receive 4 BPP */
        data.setLittleEndian16(1); /* Receive 8 BPP */
        data.setLittleEndian16(this.opt.width); /* Desktop width */
        data.setLittleEndian16(this.opt.height); /* Desktop height */
        data.setLittleEndian16(0); /* Pad */
        data.setLittleEndian16(1); /* Allow resize */
        data.setLittleEndian16(this.opt.bitmap_compression ? 1 : 0); /*
                                                                     * Support
                                                                     * compression
                                                                     */
    ...
    private void sendOrderCaps(RdpPacket_Localised data) {
        ...
        order_caps[3] = (byte) (this.opt.bitmap_caching ? 1 : 0); /* memblt */
        ...
        order_caps[21] = (byte) (this.opt.polygon_ellipse_orders ? 1 : 0); /* polygon2 */
        order_caps[22] = 1; /* polyline */
        order_caps[25] = (byte) (this.opt.polygon_ellipse_orders ? 1 : 0); /* ellipse */
        order_caps[26] = (byte) (this.opt.polygon_ellipse_orders ? 1 : 0); /* ellipse2 */
	...
	int cacheLvl2Size = this.cache.getCacheSize(2);
        if (this.opt.persistent_bitmap_caching) {
		this.opt.persistent_caching_max_cells = cacheLvl2Size;
		
		cacheLvl2Size |= BMPCACHE2_FLAG_PERSIST;

		this.common.persistent_cache.pstcache_init(2);
		logger.info("Persistent cache initialized");
	}

        data.setLittleEndian32(cacheLvl2Size);
    ...
    protected void processBitmapUpdates(RdpPacket_Localised data)
        ...
        for (int i = 0; i < n_updates; i++) {
            ...
            if (this.opt.server_bpp != bitsperpixel) {
                logger.warn("Server limited colour depth to " + bitsperpixel
                        + " bits");
                this.opt.set_bpp(bitsperpixel);
            }
            ...
            if (compression == 0) {
                ...
                surface.displayImage(bmp.convertImage(pixel, Bpp), width,
            ...
            if (Bpp == 1) {
                ...
                if (pixel != null)
                    surface.displayImage(bmp.convertImage(pixel, Bpp),
                            width, height, left, top, cx, cy);
                else
                    logger.warn("Could not decompress bitmap");
            } else {

                if (this.opt.bitmap_decompression_store == Options.INTEGER_BITMAP_DECOMPRESSION) {
                    int[] pixeli = bmp.decompressInt(width, height, size,
                            data, Bpp);
                    if (pixeli != null)
                        ...
                        logger.warn("Could not decompress bitmap");
                } else if (this.opt.bitmap_decompression_store == Options.BUFFEREDIMAGE_BITMAP_DECOMPRESSION) {
                    Image pix = bmp.decompressImg(width, height, size, data,
                            Bpp, null);
                    if (pix != null)
    ...
    public void registerDrawingSurface(RdesktopFrame fr) {
        this.frame = fr;
        this.registerDrawingSurface(fr.getCanvas());
    }
