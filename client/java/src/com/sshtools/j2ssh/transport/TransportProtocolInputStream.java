...
class TransportProtocolInputStream {
   ...
   * @throws IOException
   */
  	protected final int readBufferedData(byte[] buf, int off, int len) throws IOException 
	  	{
	    int read;
	    if ((endpos - startpos) < len) 
			...
			if ((buffered.length - endpos) < len) 
				{
				endpos -= startpos;
				System.arraycopy(buffered, startpos, buffered, 0, endpos);
				startpos = 0;
			...
			// If there is not enough data then block and read until there is
			// (if still connected)
