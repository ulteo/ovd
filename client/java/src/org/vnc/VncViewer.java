...
import org.vnc.AuthPanel;
import org.vnc.ButtonPanel;
import org.vnc.ClipboardFrame;
import org.vnc.DesCipher;
import org.vnc.ODTester;
import org.vnc.OptionsFrame;
import org.vnc.PopupError;
import org.vnc.RecordingFrame;
...
public void init() {
	...
	if (isSSH) {
		try {
			...
			String message = "Online Desktop could not start for some reason.\n" +
			"Please close the window and try again.\n";
			String caption = "Connection error";
			PopupError.showError(this, message, caption, e);
			stop();
			return;
		}
	}
...
public void paint(Graphics g){}
  ...
  try{
    ...
    //  Disable the local cursor (only soft cursor visible)
        try {
        Image img = new BufferedImage(1, 1, BufferedImage.TYPE_4BYTE_ABGR);
        Cursor c = getToolkit().createCustomCursor(img, new Point(0, 0), "Dot");
        vc.setCursor(c);
        }
      catch (Throwable t) {
        t.printStackTrace();
...
//  }
  ...
  public void readParameters() {
    ...
    if (host == null) {
      ...
      if (host.equals("")) {
	fatalError("HOST parameter not specified", null);
      }
    }
...
public String readParameter(String name, boolean required) {
	...
	try {
	  ...
	  if (required) {
	    fatalError(name + " parameter not specified", e);
	  }
	  return null;
	}
      }
...
//test1234
  ...
  private final boolean K1ZFP = false;
    @Override
