...
public class PdfGroupingAlgorithms {
	 ...
	 */
	private ScanLinePair scanLineForValue(StringBuffer[] rawContents,StringBuffer line, String value,int x1,boolean isCaseSensitive,boolean matchWholeWordsOnly) {
		...
		while (pointer < end) {
			...
			if (chars[pointer] == MARKER2) {
				...
				if (includeTease) {
					...
					/***********************************************************
					 * //<end-13> String lastWordAsString=lastWord.toString();
					 * int i=lastWordAsString.indexOf(" "); int
					 * nextSpace=lastWordAsString.indexOf(" ",i+1);
					 * while(nextSpace!=-1){ i=nextSpace;
					 * nextSpace=lastWordAsString.indexOf(" ",i+1); } /
					 **********************************************************/
				...
				if (test.indexOf(value) != -1){
					...
					if(matchWholeWordsOnly)
						isWholeWord = isWholeWord(new StringBuffer(line.toString().toLowerCase()), pointer, end, value.toLowerCase());
	...
	private boolean isWholeWord(StringBuffer line, int pointer, int end, String value) {
...
//							System.out.println("against===="+i+"---"+content[i]+" "+mode+" "+writingMode[c]+" "+isUsed[i]);
	...
	public SortedMap findMultipleTermsInRectangleWithMatchingTeasers(int x1, int y1, int x2, int y2, final int rotation, 
		...
		for (int i = 0; i < highlights.size(); i++) {
			...
			highlightsWithTeasers.put(highlight, teaser);
		}
...
//				System.out.println("RETURNING EARLY");
			...
			float[] co_ords = findTextInRectangle(x1, y2, x2 + x1, y1, page_number, term, searchType);
		...
		for (int j = count - 1; j > -1; j--) {// find from top of page
			...
			if (content[i] != null) {
				...
				while (xReached < x2) {
					...
					ScanLinePair scanLinePair = scanLineForValue(rawContent, removeDuplicateSpaces(content[i]), textValue, xReached, isCaseSensitive,matchWholeWordsOnly);
			...
			if (this.includeTease) {
				...
				for (int i = 0; i < ptCount; i++) {
					if (usingMultipleTerms)
						multipleTermTeasers.add(teaserString.elementAt(i));
					else
