...
public class PdfData extends StoryData
{
	...
	/**
    public void compare(PdfData pdf_data) {


        if(max!=pdf_data.max){
            System.out.println("Max diff "+max+" "+pdf_data.max);
            //System.exit(1);

        }

        if(pointer!=pdf_data.pointer){
            System.out.println("Pointer diff "+pointer+" "+pdf_data.pointer);
            //System.exit(1);
        ...
        try{
           ...
           int l2=pdf_data.contents.length;

           if(l1!=l2){
               System.out.println("contents "+l1+" "+l2);
               //System.exit(1);
           }

           boolean failed=false;
           for(int i=0;i<l2;i++){
               float diff=f_x1[i]-pdf_data.f_x1[i];
               if(diff<0)
               diff=-diff;
               if(diff>1){
                   System.out.println("\nf_x1 Change in "+i+" "+f_x1[i]+" "+pdf_data.f_x1[i]);
                   failed=true;
               }

               diff=f_x2[i]-pdf_data.f_x2[i];
               if(diff<0)
               diff=-diff;
               if(diff>1){
                   System.out.println("\nf_x2 Change in "+i+" "+f_x2[i]+" "+pdf_data.f_x2[i]);
                   failed=true;
               }

               diff=f_y1[i]-pdf_data.f_y1[i];
               if(diff<0)
               diff=-diff;
               if(diff>1){
                   System.out.println("\nf_y1 Change in "+i+" "+f_y1[i]+" "+pdf_data.f_y1[i]);
                   failed=true;
               }

               diff=f_y2[i]-pdf_data.f_y2[i];
               if(diff<0)
               diff=-diff;
               if(diff>1){
                   System.out.println("\nf_y2 Change in "+i+" "+f_y2[i]+" "+pdf_data.f_y2[i]);
                   failed=true;
               }

               if(f_writingMode[i]!=pdf_data.f_writingMode[i]){
                   System.out.println("\nf_writingMode Change in "+i+" "+f_writingMode[i]+" "+pdf_data.f_writingMode[i]);
                   failed=true;
               }

               if(!contents[i].toString().equals(pdf_data.contents[i].toString())){
                   System.out.println("\nChange in "+i+"\n"+contents[i]+"\n"+pdf_data.contents[i]);
                   failed=true;
               }

               if((rawData[i]==null && pdf_data.rawData[i]!=null) || (pdf_data.rawData[i]==null && rawData[i]!=null) ||(rawData[i]!=null && pdf_data.rawData[i]!=null && !rawData[i].toString().equals(pdf_data.rawData[i].toString()))){
                      System.out.println("\nrawData Change in "+i+"\n"+rawData[i]+"\n"+pdf_data.rawData[i]);
                      failed=true;
               }

               if(unformattedContent[i]==pdf_data.unformattedContent[i]){
               }else if(!unformattedContent[i].toString().equals(pdf_data.unformattedContent[i].toString())){
                      System.out.println("\nunformattedContent Change in "+i+"\n"+unformattedContent[i]+"\n"+pdf_data.unformattedContent[i]);
                      failed=true;
               }
