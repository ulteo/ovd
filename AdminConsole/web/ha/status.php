...
if(isset($_POST["action"])) {
	...
	switch($action) {
		...
		default:
			unset($_POST["action"]);
			popup_error("status.php::bad action command, hacking attempt with action '".$action."'!");
	}
	unset($_POST["node"]);
...
function action_to_resources($resource) {
	popup_error("status.php::cleaning up resource requested for : '".$resource."'");
	$ret=ShellExec::exec_cleanup_resource($resource);
}

function action_to_hosts($action, $host) {
	popup_error("status.php::action to host requested standby='".$action."' to host '".$host."'");
...
function show_default($cib) {
	...
	foreach($cib_logs_ha as $log){
				popup_error($log["time"]." Resource ".$log["resource"]." FAILDED on ".$log["node"].", rc-code=".$log["rc_code"]);
	}
	page_header();
