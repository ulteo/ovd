...
class Cib {
	...
	public function load_cib() {
		...
		if (count($res)<2) {
			popup_error("cib.class.php::get_cib() - An error has occured, please check permissions on ulteo-ovd.conf or if 'cibadmin -Q' work fine");
			return false;
		}
	...
	function get_gen_attribute() {
		...
		if($racine->hasAttribute("dc-uuid")) {
			...
			popup_error("cib.class.php::get_attribute() - An error has occured, no dc_uuid found at rootnode".$racine->nodeName); 
		 }
	}
			...
			if ($c->nodeType == XML_ELEMENT_NODE) {
				if($c->hasAttribute("id")) {
					...
					if (isset($this->nodes[(String)$id])) {
						...
						popup_error("cib.class.php::extract_nodes() - cibnode instance ".$id." doesn't exist");
					}
				}
			}
		}
