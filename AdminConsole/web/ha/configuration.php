...
function getChangeConfigurationsForm() {
	$html= '<form  onsubmit="return confirm(\''.addslashes(_("You will have to change the virtual IP address and then modify the URL in your web browser with the correct IP address to reach the admin interface!")).'\');"  action="configuration.php" method="post" enctype="multipart/form-data">';
...
function getRegisteredServer($id_host,$hostname, $fqdn, $timestamp) {
	...
	$html.= '<form onsubmit="return confirm(\''.addslashes(_("When disabling the slave SessionManager, both master and server are going to restart their services to load the new configuration. It should take less than 3 minutes. During this time the HA admin interface will be disabled.")).'\');"  action="configuration.php" method="post" enctype="multipart/form-data">';
...
function query_slave($action,$m_hostname,$passwd,$s_ip) {
	...
	if (getResponseType($req)) {
		   popup_error("Activation request from ".$m_hostname." to ".$s_ip." has been send!");
	} else{
		 popup_error("An error occured when trying to send an HTTP request (registration.php)  from ".$m_hostname." to ".$s_ip);
		 return false;
	}
	return true;
}
