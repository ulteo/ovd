...
function show_step1() {
  ...
  elseif ($_SESSION['wizard']['use_users'] == 'usergroups')
	...
	$res = $_SESSION['service']->users_list_partial('', array('login')); // todo manage wsdl to be able to send no/void argument to this request
	if ($res !== null) {
  ...
  if ($sizelimit_exceeded) {
    popup_error(_('Unable to display user list: too many users'));
    $users = array();
  }
  ...
  echo '<p>'._('Are you sure that you want to create this publication?').'</p>';
	...
	if ($_SESSION['wizard']['use_users'] == 'usergroups') {  
		...
		foreach ($_SESSION['wizard']['users'] as $user_login) {
			$user = $_SESSION['service']->user_info($user_login);
			if (! is_object($user)) {
				popup_error(sprintf(_("Unable to load user '%s'"), $user_login));
				continue;
			}
	...
	if ($_SESSION['wizard']['use_apps'] == 'appgroups') {
		...
		foreach ($_SESSION['wizard']['appgroups'] as $ag_id) {
			$appgroup = $_SESSION['service']->applications_group_info($ag_id);
		...
		foreach ($_SESSION['wizard']['apps'] as $application_id) {
			$application = $_SESSION['service']->application_info($application_id);
			if (! is_object($application)) {
				popup_error(sprintf(_("Unable to import application '%s'"), $application_id));
				continue;
			}
...
function do_validate() {
	if ($_SESSION['wizard']['use_users'] == 'users') {
		...
		$g = new UsersGroup(array(
			'id' => NULL,
			'name' => $_SESSION['wizard']['user_group_name'],
			'description' => $_SESSION['wizard']['user_group_description'],
			'published' => 1)
		);
	...
	if ($_SESSION['wizard']['use_apps'] == 'apps') {
		...
		$g = new ApplicationsGroup(array(
			'id' => NULL,
			'name' => $_SESSION['wizard']['application_group_name'],
			'description' => $_SESSION['wizard']['application_group_description'],
			'published' => 1)
		);
