...
function show_default() {
	$profiles =  $_SESSION['service']->users_profiles_list();
	...
	if($nb_profiles > 0) {
		...
		foreach ($profiles as $profile) {
			...
			$users = '';
			$profile_users = array();
			if ($profile->hasAttribute('users')) {
				$profile_users = $profile->getAttribute('users');
			}
			
			$count1 = count($profile_users);
			
			foreach($profile_users as $user_id => $user_name)
				$users[] = '<a href="users.php?action=manage&amp;id='.$user_id.'">'.$user_name.'</a>';
...
function show_manage($profile_id_) {
	$profile =  $_SESSION['service']->user_profile_info($profile_id_);
	if (! is_object($profile))
		redirect('profiles.php');

	$used_users = array();
	if ($profile->hasAttribute('users')) {
		$used_users = $profile->getAttribute('users');
	}
	
	$server = $_SESSION['service']->server_info($profile->server);
	...
	foreach ($used_users as $user_id => $user_name) {
		echo '<tr>';
		echo '<td><a href="users.php?action=manage&amp;id='.$user_id.'">'.$user_name.'</a></td>';
		echo '</tr>';
	}
