...
if (file_exists(CHROOT_SPOOL.'/broken')) {
	Logger::error('main', 'Server is "broken" !');
	die('broken');
}

if (! $buf = @file_get_contents(CHROOT.'/var/run/ulteo-ovdd.pid')) {
	Logger::error('main', 'Server is "down" !');
	die('down');
}

if (! file_exists(CHROOT.'/proc/'.$buf)) {
	Logger::error('main', 'Server is "broken" !');
	die('broken');
}
