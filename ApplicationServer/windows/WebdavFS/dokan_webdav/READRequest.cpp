...
GETRequest::GETRequest(WCHAR* url, int depth): DavRequest(url) {
	this->method = L"GET";
	this->depth = depth;
	this->parser = NULL;
}
...
HRESULT GETRequest::perform() {
	...
	//set Depth
	bodyLength = lstrlen(DEFAULT_GET)*sizeof(WCHAR);

	WinHttpAddRequestHeaders(this->hRequest, depthProperty.c_str(), (DWORD)-1, WINHTTP_ADDREQ_FLAG_REPLACE|WINHTTP_ADDREQ_FLAG_ADD);
	bResults = WinHttpSendRequest(this->hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, DEFAULT_GET, bodyLength, bodyLength, 0);
...
HRESULT GETRequest::getResult(list<DavEntry> &result) {
	if (! this->hRequest) 
		return E_FAIL;
